...
//
// Kadogo — Dala Route dark theme.
// All monetary totals come from DashboardController (server-side only).
// Dummy defaults render when no backend props are passed.
...
function TrendBadge({ today, yesterday }: { today: number; yesterday: number }) {
    ...
    return (
        <span
            ...
            style={{ fontFamily: K.mono, background: K.greenDark, color: K.green }}
        >
...
function OverviewTab({
    ...
    return (
        <div className="flex flex-col gap-5">
            ...
            {/* hero */}
            <div
                className="rounded-xl px-6 py-5 flex justify-between items-center gap-4 flex-wrap"
                style={{
            ...
            >
                <div>
                    <span
                        ...
                        style={{ fontFamily: K.mono, color: K.green }}
                    >
                        Today's takings
                    </span>
                    <span
                        className="text-4xl sm:text-5xl font-bold leading-none block"
                        style={{ fontFamily: K.mono, color: K.text }}
                    >
                        {kes(todayTotal)}
                    </span>
                ...
                <div className="flex gap-8">
                    <div>
                        <span className="text-[10px] uppercase tracking-widest font-bold block mb-1" style={{ fontFamily: K.mono, color: K.green }}>
                            Receipts
                        </span>
                        <span className="text-2xl font-bold" style={{ fontFamily: K.mono, color: K.text }}>
                            {txnCount}
                        </span>
                    </div>
                    <div>
            ...
            {flagged > 0 && (
                <div
                    ...
                    style={{
                        background: K.warnBg,
                        border: `1px solid ${K.warnBdr}`,
                    }}
                    role="alert"
                >
                    <span style={{ color: K.warnC, marginTop: 1 }}><AlertIcon /></span>
                    <div>
                        <p className="font-bold text-[13px]" style={{ color: K.warnC, fontFamily: K.display }}>
                            {flagged} duplicate receipt{flagged > 1 ? 's' : ''} blocked
                        </p>
                        <p className="text-[12px] mt-0.5" style={{ fontFamily: K.mono, color: K.warnSub }}>
                            Callbacks rejected — your total is unaffected.
                        </p>
                    </div>
                </div>
            )}

            <h2 style={{ fontFamily: K.display, color: K.text }} className="font-extrabold text-[16px]">
                Today's receipts
            </h2>
    ...
    return (
        <>
            ...
            <div
                ...
                style={{ background: K.bg, color: K.text, fontFamily: K.body }}
            >
                <TopBar shop={shop} />
